					...
					<form action="student/${msg }.do" name="Form" id="Form" method="post">
						...
						<table id="table_report" class="table table-striped table-bordered table-hover">
							...
							<tr>
								...
								<td>
							        <div  style="">
							            <select class="chosen-select form-control" name="SCHOOL_ID" id="SCHOOL_ID" data-placeholder="请选择" style="width:98%;">
								            <option value="">请选择</option>
								            <c:choose>
												<c:when test="${not empty varSchoolList}">
													<c:forEach items="${varSchoolList}" var="varSchool" varStatus="vsSchool">
														<c:choose>
															<c:when test="${not empty pd.SCHOOL_ID && pd.SCHOOL_ID==varSchool.SCHOOL_ID}">
																<option value="${ varSchool.SCHOOL_ID}" selected>${ varSchool.SCHOOL_NAME}</option>
															</c:when>
															<c:otherwise>
																<option value="${ varSchool.SCHOOL_ID}">${ varSchool.SCHOOL_NAME}</option>
															</c:otherwise>
														</c:choose>
													</c:forEach>
												</c:when>
